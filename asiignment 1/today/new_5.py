...
for j in l1:
	...
	print(df["tid"],df["like_count"],df['date'])
	session.execute(
	 """
		 INSERT INTO mytable (tid, like_count,date)
		 VALUES (%s, %s,%s)
	 """,
	(df["tid"],df["like_count"],df['date'])
	 )
	
	
future = session.execute_async("select * from mytable where date = '2017-11-26' order by like_count DESC;")
