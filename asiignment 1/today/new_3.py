	...
	if df["hashtags"] is not None :
		for k in df["hashtags"] :
			if k :
				...
				session.execute(
			 	 """
					 INSERT INTO mytable (tid, hashtags,date)
					 VALUES (%s, %s,%s)
			 	 """,
			  	(df["tid"], k,df['date'])
		       		 )
future = session.execute_async("select * from mytable where hashtags ='Sindhi' order by date DESC")
