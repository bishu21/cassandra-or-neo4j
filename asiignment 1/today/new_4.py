	...
	if df["mentions"] is not None :
		for k in df["mentions"] :
			if k :
				...
				session.execute(
			 	 """
					 INSERT INTO mytable (tid, mentions,date)
					 VALUES (%s, %s,%s)
			 	 """,
			  	(df["tid"], k,df['date'])
		       		 )
 future = session.execute_async("select * from mytable where mentions ='kylegriffin1' order by date DESC")
